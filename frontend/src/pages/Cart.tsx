        ...
        {items.map((item) => (
          <div key={item.productId} className="grid grid-cols-12 gap-4 py-6 items-center">
            ...
            <div className="col-span-4 md:col-span-2 flex items-center justify-center gap-2">
              <button onClick={() => updateQty(item.productId, Math.max(1, item.quantity - 1), item.itemId)}
                className="w-8 h-8 border border-brand-200 flex items-center justify-center text-brand-700 hover:bg-brand-100 transition-colors text-sm">&minus;</button>
              <span className="w-8 text-center text-sm font-medium text-brand-900">{item.quantity}</span>
              <button onClick={() => updateQty(item.productId, Math.min(item.quantity + 1, item.stockQty), item.itemId)}
                className="w-8 h-8 border border-brand-200 flex items-center justify-center text-brand-700 hover:bg-brand-100 transition-colors text-sm">+</button>
            </div>
